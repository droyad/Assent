...
public class RiderDiffProgram : DiffProgramBase
{
    ...
    static RiderDiffProgram()
    {
        ...
        var possibleExePaths = Directory.GetDirectories(riderChannelsDirectory).Select(GetRiderExePathInChannel).ToList();

        DefaultSearchPaths = possibleExePaths;
    }

    public RiderDiffProgram()
